document.addEventListener('DOMContentLoaded', function() {
  ...
  function initLanguageSwitcher() {
    ...
    // Check if any browser language is Arabic
    const isArabicBrowser = userLangs.some(lang => 
      lang.startsWith('ar') || 
      lang.includes('ar') || 
      lang.toLowerCase().includes('arabic')
    ) || userLang.startsWith('ar') || userLang.includes('ar');
    
    if (isArabicBrowser) {
      currentLang = 'ar';
      console.log('Arabic browser detected, setting language to Arabic');
    }

    // Check for stored language preference (only use if explicitly set by user)
    const storedLang = localStorage.getItem('preferredLanguage');
    const hasUserSetLanguage = localStorage.getItem('userSetLanguage') === 'true';
    
    if (storedLang && hasUserSetLanguage) {
      currentLang = storedLang;
      console.log('Using stored language preference:', storedLang);
    } else if (isArabicBrowser) {
      // Clear any old stored preference if browser is Arabic but user hasn't explicitly set language
  ...
  function addLanguageSwitcherListeners() {
    ...
    languageSwitchers.forEach(link => {
      link.addEventListener('click', function(e) {
        ...
        setLanguage(lang, true); // true indicates user manually set the language
      });
    });
  }
  ...
  function setLanguage(lang, userSet = false) {
    ...
    // Store language preference only if user manually set it
    if (userSet) {
      localStorage.setItem('preferredLanguage', lang);
      localStorage.setItem('userSetLanguage', 'true');
      console.log('User manually set language to:', lang);
    }
  }
}); 